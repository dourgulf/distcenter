...
function parseInfoPlist(ipaInfo, fn) {
    ...
    fs.readFile(infoPlist, function (err, plistData) {
        ...
        var cmdConvertIcon = "xcrun pngcrush -revert-iphone-optimizations " + path.join(basepath, "icon_iOS.png") + " " + path.join(basepath, "icon.png");
        debug("convert icon command:" + cmdConvertIcon)
        exec(cmdConvertIcon, function (error, stdout, stderr) {
            if (error) {
...
function parse(ipaInfo, fn) {
    var basepath = ipaInfo.basePath;
    fs.createReadStream(path.join(ipaInfo.basePath, ipaInfo.fileName))
